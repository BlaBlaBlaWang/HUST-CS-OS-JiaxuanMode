...
typedef struct shmlink shmlink;
shmlink shared[10];
...
int main(void)
{
	...
	for(int i=0;i<10;i++)
	{
		...
		shared[i].sharedmemory=sharedmemory[i];
		shared[i].next=&(shared[(i+1)%10]);
	}	
	...
	if(writebuf==0)
	{
		...
		for(;tempchar!=EOF;)
		{
			...
			for(i=0;i<10;i++)
			{
				if((tempchar=*(tempshared.sharedmemory+i))==EOF)
					break;
				fputc(tempchar,destination);
			}

			tempshared=*(tempshared.next);
		...
		if(readbuf==0)
		{
			...
			do
			{
				for(int i=0;i<10&&tempchar!=EOF;i++)
				{
					...
					*(tempshared.sharedmemory+i)=tempchar;
				}
