...
void printdir(char *dir,int depth,char *fulldir){
	...
	struct dirent*tempdirent;
	struct stat tempstate;
    vector <struct dirent> foldertoprint;
	...
	while((tempdirent=readdir(dp))!=NULL){
		...
		if(judge(tempdirent->d_name))
			continue;
		wholesize+=(tempstate.st_size%4096)?tempstate.st_size/4096+1:tempstate.st_size/4096;
	}
    printf("total %d\n",wholesize*4);
    chdir("..");	//back to the parent direction
    closedir(dp);	//close the opened dirent flow
	...
	while((tempdirent=readdir(dp))!=NULL){
		...
		if(S_ISDIR(tempstate.st_mode)){	
            ...
            foldertoprint.push_back(*tempdirent);
		}else{
    ...
    while(foldertoprint.size()!=0)
    {
			printdir(foldertoprint.front().d_name,depth+4,relpath);
            foldertoprint.erase(foldertoprint.begin());
    }
	chdir("..");	//back to the parent direction
	closedir(dp);	//close the opened dirent flow
